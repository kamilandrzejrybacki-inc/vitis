# Vitis Agent Bridge - Implementation Plan

**Design Spec**: `docs/superpowers/specs/2026-04-03-vitis-agent-bridge-design.md`
**Date**: 2026-04-03
...
- `go mod init github.com/kamilandrzejrybacki-inc/vitis`
- Create directory structure under `cmd/vitis/` and `internal/`
...
**Done when:** `vitis run` and `vitis peek` work end-to-end against the mock PTY provider.
...
- Spawn `vitis run` against mock agent in happy mode
...
- Run with `--log-backend file`, then `vitis peek` on same session
- Verify turns match

### Step 8.5: Postgres backend round-trip

- Testcontainers Postgres
- Run with `--log-backend db`, then `vitis peek`
- Verify turns match
...
- Run `vitis run --provider claude-code --prompt "What is 2+2?"` against real Claude Code
