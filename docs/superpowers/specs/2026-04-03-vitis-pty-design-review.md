# Vitis PTY Design Review And Safeguards

- **Status**: Draft
- **Date**: 2026-04-04
- **Author**: Codex / Claude-code-leak review
- **Scope**: PTY-only design review for Vitis
...
This document reviews Vitis's current PTY-based design against:

- the current Vitis brainstorm docs
...
The goal is not to change Vitis into a different product. The goal is to make the PTY-only design safer, less brittle, and less likely to drift into a product shape that creates account, policy, or operational risk.

---

## 2. Executive Summary

The current Vitis design is directionally sensible as a local PTY orchestrator, but it is too optimistic in four areas:
...
- `docs/superpowers/specs/2026-04-03-vitis-agent-bridge-design.md`
- `docs/superpowers/specs/2026-04-03-vitis-implementation-plan.md`
...
### 4.2 Practical reading for Vitis
...
- A hosted service, daemon, browser-accessible PTY, bot platform, or team product that lets other people drive Claude Code through Vitis while using Claude.ai/Pro/Max/consumer auth.
...
Vitis should adopt the following product boundary in writing:

- **Supported MVP**: local, one-user, local-machine PTY automation around the user's own Claude Code installation.
- **Explicitly unsupported**:
  - multi-tenant hosted Vitis using Claude consumer auth
...
## 5. Main Design Gaps In The Current Vitis Spec
...
Vitis should preserve raw bytes first, then derive normalized text second.
...
This is not enough if Vitis ever runs repeated sessions, parallel sessions, or any shared/server mode.
...
Vitis does not need all of that in MVP, but it does need a clearer stance on:

- whether `HOME` is inherited or isolated
- whether `.claude` state is shared across runs
- whether multiple Vitis runs can contend on one Claude Code auth/config directory
...
Before adding a DB backend, Vitis needs:
...
Without that, the adapter tests will only prove that Vitis works against Vitis's mock agent, not against the messy states a real Claude Code PTY produces.
...
This should be ported almost directly into Vitis's Claude provider adapter.
...
If Vitis ever grows beyond a local CLI, these are table stakes.
...
Vitis does not need browser cookies, but it does need the same attitude:
...
Vitis should mirror this principle:
...
But the concept needs to exist early so Vitis can later isolate sessions instead of baking in hidden shared-state assumptions.

### 7.7 Make "local-only" an explicit product constraint

Add to the design spec:

- Vitis MVP is a local CLI for one user on one machine.
- Vitis MVP is not a hosted PTY broker.
- Vitis MVP does not expose browser sessions or shared session IDs.
...
- "A platform that lets teams use Claude Code through Vitis"
...
> Vitis is a local PTY orchestrator for AI agent CLIs. It is designed for local execution against a user's own installed CLI and account context. Vitis MVP does not provide hosted, pooled, or multi-tenant access to Claude Code or other subscription-backed consumer AI products.

Suggested text:

> In PTY environments, "no output" is not sufficient evidence of completion. Vitis distinguishes successful completion from blocked interactive prompts, authentication requirements, permission prompts, and rate-limit states.

Suggested text:

> Vitis stores raw PTY bytes as the source of truth and derives normalized text for parsing and display. Parsing must not depend on lossy text conversion of the capture stream.
...
1. Will Vitis ever be allowed to pass through `HOME`, or must it always isolate `.claude` state per run?
2. Will Vitis ever intentionally support more than one concurrently active Claude-authenticated session for one local user?
3. If Claude Code prompts for login, should Vitis:
   - fail immediately
   - classify `auth_required`
   - or support a future attach/resume flow?
4. Does Vitis need a `doctor` or `probe` command to capture provider fingerprints and transcript markers before normal runs?
...
Proceed with Vitis as a PTY-only local tool if and only if the project adopts these principles:
