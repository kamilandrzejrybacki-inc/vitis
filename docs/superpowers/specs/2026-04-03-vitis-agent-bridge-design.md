# Vitis Agent Bridge - Design Spec
...
Vitis is a Go CLI tool that drives AI agent CLIs (Claude Code, OpenCode, etc.) through a simulated terminal (PTY). It sends a prompt, detects when the agent is done, extracts the final response, logs the session, and outputs structured JSON.
...
- One PTY process per `vitis run` invocation
...
HOME, `.claude`, environment variables, and the working directory are all caller-controlled. Vitis passes them through to the spawned process without modification. There is no sandboxing layer in MVP.
...
- **Happy path**: `vitis run` against mock provider, verify JSON output, session and turns in store.
...
- **File backend**: verify JSONL written, `vitis peek` reads correctly.
