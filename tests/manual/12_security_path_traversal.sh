...
#   - disallowed env_KEY peer options (LD_PRELOAD, CLANK_CLAUDE_ARGS) are
...
err=$( "${CLANK}" converse \
  ...
  --peer-a-opt env_CLANK_CLAUDE_ARGS=--dangerously-skip-permissions \
...
if grep -q "CLANK_CLAUDE_ARGS" <<< "${err}"; then
  ok "CLANK_CLAUDE_ARGS dropped with stderr warning"
else
  fail "expected stderr warning about CLANK_CLAUDE_ARGS"
fi
